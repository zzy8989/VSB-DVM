...
parser.add_argument('--lr', type=float, default=0.002, metavar='N',
                    help='learning rate.')
parser.add_argument('--gmmlr', type=float, default=1.5, metavar='N',
...
# torch.manual_seed(684232159687)
torch.manual_seed(684232159687)
# torch.manual_seed(684232159687)  #86.61 1.5  lr = max(args.lr * 0.9**(np.floor(epoch-0)/16.0), 0.00002) 285/300
# torch.manual_seed(683631782)

load_fn = './data/HAR.mat'
...
class VAE(nn.Module):
    def __init__(self, lr = 0.002, gmm_lr = 1, n_gmm = 6, latent_dim=30):
        super(VAE, self).__init__()

        layers = []
        layers += [nn.Linear(561, 500)]
        layers += [nn.ReLU()]
        layers += [nn.Linear(500, 1000)]
        layers += [nn.ReLU()]
        ...
        layers += [nn.Linear(1000, 500)]
        layers += [nn.ReLU()]
        layers += [nn.Linear(500, 561)]
        # layers += [nn.Sigmoid()]
        #layers += [nn.Linear(561, 561)]
        self.IFC = nn.Sequential(*layers)

        self.pi = torch.nn.Parameter(data=torch.ones(n_gmm)/n_gmm, requires_grad=False)
        self.mu = torch.nn.Parameter(data=torch.randn(n_gmm, latent_dim)/10.0, requires_grad=False)
        self.var = torch.nn.Parameter(data=torch.ones(n_gmm, latent_dim)/10.0, requires_grad=False)
...
# checkpoint = torch.load('./checkpoint/ckpt8-9har5_best.t8-9')#
...
def train(epoch):
    ...
    for batch_idx, (data, gt) in enumerate(train_loader):
        lr = args.lr
        if epoch>=0:
            lr = max(args.lr * 0.9**(np.floor(epoch-0)/15.0), 0.0006)
            # if epoch > 200:
                # lr = lr * ((np.cos(((epoch-200)/(math.pi*2)))+1)/2)
            #lr = args.lr * ((np.sin((epoch/(math.pi*2)))+1)/2)
            for param_group in optimizer.param_groups:
                param_group['lr'] = lr
        gmm_lr = args.gmmlr#min(epoch/2.0+1, 100)*0.1
    ...
    if epoch % 10 == 0:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt8-9har5.t8-9')
    ...
    if (acc>ac):
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ckpt8-9har5_best.t8-9')
